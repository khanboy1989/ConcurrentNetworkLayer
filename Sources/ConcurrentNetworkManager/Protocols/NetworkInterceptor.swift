...
//
//  Created by Serhan Khan on 10/11/2025.
//
...
public protocol NetworkInterceptorProtocol: Sendable {
    ...
    /// Intercepts and potentially modifier and outgoing network request before it is sent.
    ///
    /// - Parameter request: The original `URLRequest` to be sent.
    /// - Returns: A modified `URLRequest` that will be executed.
    func intercept(request: URLRequest) -> URLRequest
    
    /// Intercepts and potentially modifier the response received from the network.
    ///
    /// - Parameters:
